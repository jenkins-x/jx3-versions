...
export CLUSTER_NAME="${BRANCH_NAME,,}-$BUILD_NUMBER-bdd-kube"
export ZONE=europe-west1-c
export LABELS="branch=${BRANCH_NAME,,},cluster=bdd-kube,create-time=${CREATED_TIME,,}"
...
jxl boot create -b --env dev --provider=kubernetes --version-stream-ref=$PULL_PULL_SHA --env-git-owner=$GH_OWNER --project=$PROJECT_ID --cluster=$CLUSTER_NAME --zone=$ZONE
